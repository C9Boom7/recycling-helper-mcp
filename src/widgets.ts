...
import { disposalGroupLabel, itemNeedsRegionCheck } from "./data.js";
 ...
 * region line at all.
 */
function regionNodes({ item, regionName, regionNotes }: DisposalWidgetInput): WidgetNode[] {
  if (!itemNeedsRegionCheck(item)) return [];

  if (regionName && regionNotes && regionNotes.length > 0) {
    return [
  ...
  if (regionName) {
    return [text(`${regionName} 기준으로 배출 요일·장소만 확인하면 됩니다.`)];
  }

  return [text("거주 지역 기준 확인 필요")];
}

export function buildDisposalWidget(input: DisposalWidgetInput): DisposalWidgetPayload {
  const { item, sourceTitle } = input;
  const cautions = item.cautions.slice(0, MAX_CARD_CAUTIONS);
...
export function buildCopyText(item: WasteItem): string {
  const steps = item.steps.slice(0, MAX_COPY_TEXT_STEPS);
  return [`**${item.name} 버리는 법** — ${SERVICE_LABEL}`, ...steps.map((step, index) => `${index + 1}. ${step}`)].join("\n");
}
