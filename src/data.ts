...
export function formatItemGuide(item: WasteItem, region?: string): string {
  ...
  const lines = [
    `## ${item.name}`,
    "",
    // 결론을 라벨 없이 제목 바로 밑에 세운다. 예전에는 `- 결론:`이라는 라벨을 달고 `배출
    // 그룹` 다음 네 번째 줄에 있었는데, 카카오톡 말풍선에서 처음 두 줄이 가장 많이 읽히는
    // 자리라 거기에 분류 메타데이터가 서고 정작 답이 아래로 밀려 있었다.
    item.summary,
    "",
    ...
    // 105개를 통제 어휘로 묶어 계속 관리하겠다는 뜻인데, 그 대가로 얻는 줄이 바로 위
    // 결론 문장보다 사용자에게 덜 말해 준다.
    //
    ...
    // 갈리는 답에만 남겨야 그 표시가 실제로 신호가 된다. 위젯 카드(`buildDisposalWidget`)가
    // 이미 같은 조건으로 확신도 단서를 걸고 있어, 두 표면이 이제 같은 규칙을 쓴다.
    item.confidence === "high" ? undefined : `- 확신도: ${confidenceLabel(item.confidence)}`,
    // 지역 블록(`### 지역 확인 필요`·`### 지역 참고`)이 아래에 설 응답에서는 뺀다 — 그 블록이
    // 같은 말을 더 자세히 해서 두 번 말하는 자리가 된다. 블록이 안 서는 갈래에는 둘이 있다.
    // 지역을 안 타는 품목은 "전국 공통 기준으로 바로 안내 가능합니다"가 되고, 참고 수준인데
    // 지역을 안 준 호출은 "기본 분리배출 판단은 전국 기준으로…"가 된다. 둘 다 되묻기를
    // 줄이는 값이라 남긴다.
