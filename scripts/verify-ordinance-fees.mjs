 ...
 *
 * 수기 데이터를 그대로 들고 있는 3개 지역(강남·서초·송파, 55행)을 조례에서 다시 뽑아
 * `src/data/bulky-waste-fees.json`과 대조한다. 파서를 새 지역에 쓰기 전에 여기부터
 * 통과시킨다.
 *
 * 넷이던 대상이 2026-08-19에 셋이 됐다. 마포가 조례 추출로 출처를 옮기면서 빠졌다 —
 * 우리 행이 이 검증이 다시 돌리는 그 파서에서 나오면 대조가 자기 충족이 되기 때문이다.
 * 자세한 사정은 `fetch-ordinance-fees.mjs`의 `GOLDEN_TARGETS` 주석에 적어 뒀다.
 *
...
async function main() {
  ...
  // 되는지에 바닥을 둔다 — 실측 47/55(85%)라 절반은 넉넉한 선이다.
