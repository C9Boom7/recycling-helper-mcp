...
const TARGETS = [
  ...
  "mapo_gu",
  // 서초(2026-08-26). 위 "서초" 문단이 미룬 이유였던 두 가지가 다 풀렸다.
  //   - 요가매트·리빙박스가 조용히 사라지는 문제: 2024-12-19 시행 별표에는
  //     「대자리(돗자리) 3.3㎡당 2,000원」과 「서랍장 4단 이하 2,000 / 5단 이상 4,000」이
  //     있어 두 품목이 조례 행으로 제자리를 찾는다. 임포트 뒤 품목 집합을 대조해
  //     없어지는 품목이 없음을 확인했다.
  //   - 골든셋이 비는 문제: 수기 36행을 `scripts/fixtures/ordinance-golden-rows.json`
  //     으로 동결해, 운영 데이터를 채워도 파서 대조 기준이 남는다.
  "seocho_gu",
];
