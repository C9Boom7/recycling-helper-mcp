 ...
 *
 * 막는 뜻은 그 행을 원래 자리인 `bed_frame`에 두라는 것이다. 버리지 않는 이유는 세트가
 * 실제로 존재하는 배출 방식이고, 규격 칸이 「1인용 세트 (매트리스1개+틀)」처럼 스스로를
 * 설명해서 후보 목록에서 프레임만의 값(5,000원)과 구별되기 때문이다. 매트리스 쪽으로
 * 가는 것만 막으면 리뷰가 짚은 해악 — 매트리스만 버리는 사람이 세트 값을 받는 것 — 은
 * 사라진다.
 *
 * **`deny`만으로는 행이 남지 않는다.** 임포터에는 규격 칸이 다른 품목으로 풀리면 행을
 * 버리는 검사가 따로 있어서, `deny`가 힌트를 끊어도 그 검사가 같은 낱말을 다시 보고
 * 행을 통째로 버린다. 그래서 임포터가 `deny`에 막힌 규칙의 `to`를 조각 검사에서
 * 빼 준다(`import-district-fees.ts`). 규격 칸이 괄호 안에 있는 강남 세트 행은
 * 괄호가 떨어져 나가 조각 검사에 안 걸렸고, 그래서 이 함정이 광주 북구를 넣기 전까지
 * 드러나지 않았다.
 *
 * 2026-08-23에 대안을 하나 더 얹었다. 광주 북구는 프레임 단독 행을 「2인용 매트리스 틀
 * 9,000원」으로 적는다 — `매트리스`가 들어 있어 그대로 `mattress`로 갔고, 매트리스를
 * 물은 사람이 프레임 값 6,000·9,000원을 후보로 받았다(실제 2인용 매트리스는 11,000원).
 * `매트리스` **바로 뒤에 붙은 `틀`**만 막는다. 괄호를 사이에 두는 건 허용하지 않는다 —
 * 「매트리스(틀 제외)」는 매트리스 행이라 그것까지 막으면 규칙이 뒤집힌다. 같은 이유로
 * 뒤에 `제외`·`별도`가 따라오면 발동하지 않는다. 받아 둔 원문 전수에서 이 표기는
 * 광주 북구 세 행뿐이고, 셋 다 프레임이 맞다.
 *
...
export const SPLIT_HINTS: Array<{ from: string; hint: RegExp; to: string; deny?: RegExp }> = [
  {
    ...
    hint: /매트리스|토퍼/,
    to: "mattress",
    deny: /(매트리스|토퍼)\s*[)）]?\s*(제외|미포함|불포함|없음|포함|별도)|세\s*트|매트리스[^)）]*[+＋]\s*틀|틀\s*[+＋][^)）]*매트리스|매트리스\s*틀(?!\s*[)）]?\s*(제외|별도|미포함|불포함|없음))/,
  },
 ...
 * 마포가 그 값을 치렀다 — 조례에 돗자리 행이 있는데도 요가매트가 금액을 잃었다. 이어서
 * 조례 임포터가, 2026-08-23부터는 표준데이터 임포터(`import-bulky-fees.ts`)도 쓴다 —
 * 관악 「인형+장난감류」가 장난감에만 붙고 인형은 금액을 잃고 있었다. 세 트랙이 전부 같은
 * 표를 본다. 새 트랙이 생기면 여기 적고 그쪽도 붙인다.
 */
