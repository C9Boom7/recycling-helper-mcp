...
export const TARGETS = [
  ...
  // 서초구(2026-08-26). 골든셋을 픽스처로 동결한 뒤 열었다 — 그전에는 수기 18행이
  // 조례 파서를 대조하는 유일한 기준이라 채우는 순간 검증이 죽었다.
 ...
 * `pnpm fees:fetch`가 실수로 골든셋을 덮어쓰지 않게 하려던 것이다. **2026-08-26에 그 불변식은
 * 끝났다** — 골든셋이 운영 데이터가 아니라 `scripts/fixtures/ordinance-golden-rows.json`으로
 * 옮겨 가서, 원문 덤프를 다시 받아도 대조 기준은 안 흔들린다. 실제로 서초는 지금 두 목록에
 * 함께 들어 있다(수집 대상이면서 검증 대상). 두 목록을 여전히 갈라 두는 건 역할이 달라서다.
 *
