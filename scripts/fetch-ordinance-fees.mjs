 ...
 *
 * 2026-08-19부터 넷이 두 몫으로 갈린다. 목록은 그대로 두되 읽는 법이 다르다.
 *
 * - **강남·서초·송파는 여전히 독립 기준이다.** 수기 데이터를 그대로 들고 있어서,
 *   여기 불일치가 0으로 유지되는 동안만 파서를 믿을 근거가 된다.
 * - **마포는 조례 추출로 갈아탔다**(`import-ordinance-fees.ts` 참고). 수기 14행이
 *   Top 50 범위뿐이라 침대 프레임조차 없었기 때문이다. 그래서 마포의 대조는 더 이상
 *   파서 검증이 아니라 **조례 개정 감지**다 — 우리 스냅샷과 현행 조례의 금액이
 *   갈리면 개정이 있었다는 뜻이다. 파서를 새로 고칠 때 믿을 기준으로 쓰지 마라.
 */
