...
/**
 * R2 골든셋. 이 셋은 이미 검증된 수기 데이터가 있어 파서를 대조할 기준이 된다.
 * 추출 대상이 아니라 검증 대상이므로 TARGETS와 분리해 둔다 —
 * `pnpm fees:fetch`가 실수로 골든셋을 덮어쓰지 않게 하려는 것이다.
 *
 * 2026-08-19에 넷에서 셋이 됐다.
 *
 * - **강남·서초·송파는 여전히 독립 기준이다.** 수기 데이터를 그대로 들고 있어서,
 *   여기 불일치가 0으로 유지되는 동안만 파서를 믿을 근거가 된다.
 * - **마포는 목록에서 뺐다.** 조례 추출로 갈아탄 순간 우리 행이 이 검증이 다시 돌리는
 *   그 파서에서 나온 것이 되어, R2의 "파서가 골든셋을 읽는다" 하한선이 **자기 충족**이
 *   된다 — 195행 중 140행이 늘 맞아서, 나머지 셋이 0개 맞아도 하한선이 안 깨진다.
 *   개정 감지가 필요하면 `pnpm fees:verify mapo_gu`가 아니라, 마포를 인자로 받는 별도
 *   대조가 필요하다(지금은 없다). 그 몫은 `import-ordinance-fees.ts` 주석에 남겼다.
 */
