...
리포트는 세기만 하고 판정하지 않는다 — 문턱을 넘었다고 실패로 끝내면 정작 볼 숫자를 못 보게 된다. 다만 **줄은 있는데 하나도 못 읽었으면** 형식이 바뀐 것이므로 그때는 끊는다. 형식은 `logs/call-log.example.jsonl`에 예시가 있고, `pnpm check`가 그 파일로 스크립트가 살아 있는지 확인한다.
