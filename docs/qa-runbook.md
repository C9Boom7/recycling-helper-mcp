...
**어떤 품목이 안 잡혔는지는 운영에서 알 길이 없다.** 운영 로그는 애초에 볼 수 없고(2절), 로컬에서 재현해 로그를 받아도 품목명은 안 남는다. 그 로그에서 건질 수 있는 건 `fallbackTier`뿐이고, `menu` 비중이 높으면 재질 추정조차 못 하는 질의가 많다는 뜻이다. 품목 자체는 문의 내용이나 로컬 테스트에서 모은다 — `pnpm log:query -- --query "..."`로 쌓고 `pnpm backlog:auto -- --input logs/manual-queries.jsonl`로 후보를 뽑는다.
...
- 운영 로그를 직접 볼 수 없으므로(2절) 호출 여부는 Preview에서 재현해 잡는다. **"Kakao Tools 확인 중" 지표가
  뜨는지가 판정 근거다.** 안 뜨면 호스트가 툴을 고르지 않은 것이다.
- **202가 없다고 서버까지 안 왔다고 단정하면 안 된다.** 202는 세션 초기화(`notifications/initialized`)에만 나가고
  ...
  202 버스트는 세션이 언제 돌았는지 교차 확인하는 용도까지다.
