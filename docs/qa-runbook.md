...
1. **한 번도 안 불린 툴.** 이 리포트에서 가장 중요한 줄이다. 데이터가 아무리 좋아도 호스트가 그 툴을 안 고르면 사용자에게 닿지 않는다. 프리징 전이면 description을 손볼 마지막 신호다.
2. **답을 못 준 비율.** `not_found`·`ambiguous`가 어느 툴에 몰리는지 본다. 품목 데이터를 어디에 더 넣을지가 여기서 나온다 — 품목명은 로그에 안 남으므로, 무엇을 넣을지는 3절 ①의 절차로 따로 좁힌다.
3. **지역 해상도.** `unregistered_district`가 크면 사용자가 시·군·구를 대는데 우리에게 그 지역이 없다는 뜻이고, `unknown`이 크면 아예 못 알아들은 것이다. 둘은 대응이 다르다.
4. **외부 API.** `body`가 쌓이면 인증키 오류나 일 한도 초과다. `network`·`http`와 원인이 다르니 5절로 간다.
5. **지연.** 서버 처리 시간과 업스트림 지연을 갈라 본다. `find_disposal_spots`만 300~500ms가 정상이고 나머지는 한 자릿수다.

리포트는 세기만 하고 판정하지 않는다 — 문턱을 넘었다고 실패로 끝내면 정작 볼 숫자를 못 보게 된다. 형식은 `logs/call-log.example.jsonl`에 예시가 있고, `pnpm check`가 그 파일로 스크립트가 살아 있는지 확인한다.
