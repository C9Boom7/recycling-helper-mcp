...
| `regionStatus` | 지역이 어디까지 좁혀졌는지. `district`(자치구 확정) / `unregistered_district`(시·군·구를 댔지만 상세 데이터 없음) / `metro`(광역만 지목) / `ambiguous`(후보가 여럿이라 되물음) / `unknown`(찾아봤는데 없음). **값이 없으면 그 호출은 지역을 안 봤다는 뜻이다** — 지역을 안 물었거나, 물었어도 그 품목은 지역이 답을 바꾸지 않아 조회하지 않은 경우다. `unknown`과 뭉치지 않는다 |
| `inputSource` | `photo`면 품목명이 사진에서 온 것. 없으면 사용자가 직접 친 것 |
...
- 등록된 49개 지역인지 먼저 본다. `full` 5곳은 배출 요일까지, `standard` 27곳은 신청·수수료 링크와 전화, `metro` 17곳은 광역 폴백만 담는다.
- **서울은 25개 구가 전부 등록돼 있다**(full 4곳 + standard 21곳). 서울 자치구 문의를 "미등록 지역"으로 넘기면 안 된다.
- 예외 하나: `중구`만 치면 전국 폴백으로 떨어진다. **이건 버그가 아니다.** 중구는 부산·대구·인천·대전·울산에도 있어서, 광역 접두어 없이 확정하면 다른 지역 주민이 서울 중구의 전화번호를 받는다. `서울 중구`로 다시 물으면 정상 안내가 나간다. `동구`·`서구`·`남구`·`북구`·`강서구`도 같다 — 별칭을 달아 "고치지" 마라. 자치구로 달든 광역 별칭으로 달든 검증이 실패한다. `고성군`(강원·경남)도 같은 이유로 어느 광역 별칭에도 달지 않았다.
- **`광주시`는 예외의 예외다. 전국 폴백이 아니라 광주광역시로 확정된다.** 광주광역시가 원래 `광주시`를 자기 별칭으로 갖고 있어서이고, 이번 별칭 확장이 만든 동작이 아니다. 그래서 **경기도 광주시 주민이 `광주시`라고만 치면 광주광역시 안내를 받는다.** `경기 광주시`로 치면 정상 착지한다. 경기도 별칭에 `광주시`를 더해 "고치면" 두 광역이 같은 이름을 물어 되묻기로 빠지므로 그렇게 하지 마라.
- **미등록 시·군·구도 이름만 대면 광역 안내는 받는다.** `metro` 17개가 소속 기초자치단체 이름 174개를 별칭으로 물고 있어서, `수원시`는 경기도로, `해운대구`는 부산광역시로 착지한다. 여기서 "매칭 없음"이 나오면 이름이 별칭 목록에서 빠졌거나 위의 동명 예외에 걸린 경우다.
- 자치구가 아닌 일반구 이름만 말하면(`흥덕구`, `영통구`) 아직 전국 폴백이다. `청주시 흥덕구`처럼 앞에 시 이름을 붙이면 광역으로 착지한다.
