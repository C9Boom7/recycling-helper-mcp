...
- **미등록 시·군·구도 이름만 대면 광역 안내는 받는다.** `metro` 17개가 소속 기초자치단체 이름 191개를 별칭으로 물고 있어서, `수원시`는 경기도로, `사하구`는 부산광역시로 착지한다. **자치구로 등록된 이름은 이 목록에서 빠져 있다** — Phase 9에서 해운대구·부산진구·남동구·부평구를 승격하며 광역 쪽 별칭에서 함께 걷어냈고, 남겨 두면 `pnpm check`의 naming sweep이 그 자리에서 깨진다. 여기서 "매칭 없음"이 나오면 이름이 별칭 목록에서 빠졌거나 위의 동명 예외에 걸린 경우다.
