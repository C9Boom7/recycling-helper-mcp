...
- **운영 트래픽의 활동 시간대**: 콘솔 모니터링 탭에서 202 응답이 몰리는 구간으로 MCP 세션이 언제 돌았는지만 교차 확인할 수 있다.
  응답 코드별 집계라 어느 툴이 불렸는지는 안 나온다. **게다가 이 숫자는 쉽게 오염된다.** 202는 핸드셰이크(`notifications/initialized`)
  신호라서, 우리가 Preview 탭을 붙이기만 해도 `tools/list`가 다시 돌며 202가 찍히고 검증하느라 친 트래픽도 같이 섞인다.
...
- **툴 단위 판정**: Kakao Tools에서 Preview를 만들 때 뜨는 "Kakao Tools 확인 중" 지표로 호출 여부를 잡고,
  ...
  **이 표시는 답변을 만드는 동안 2초 남짓 떴다 사라지는 순간 지표다.** 전송 직후부터 2.5초 간격으로 연속 스크린샷을 찍어야 잡힌다.
  한 번 훑고 "안 떴다"고 적으면 실제로 호출된 건을 미호출로 판정하게 된다. 연속 촬영으로 지켜봤는데도 안 떴을 때만 미호출이다.
...
- 운영 로그를 직접 볼 수 없으므로(2절) 호출 여부는 Preview에서 재현해 잡는다. **"Kakao Tools 확인 중" 지표가
  뜨는지가 판정 근거다.** 단 2초 남짓 떴다 사라지는 순간 지표라, 전송 직후부터 2.5초 간격 연속 스크린샷으로 지켜봐야 한다.
  **그렇게 지켜봤는데도 안 떴을 때만 호스트가 툴을 고르지 않은 것이다.** 한 번 훑고 "안 떴다"고 넘기면 호출된 건을 미호출로 적게 된다.
- **202가 없다고 서버까지 안 왔다고 단정하면 안 된다.** 202는 세션 초기화(`notifications/initialized`)에만 나가고
  실제 `tools/call`은 200으로 답한다. 세션을 미리 열어 둔 호스트가 나중에 부른 호출은 그 시각에 202를 남기지 않는다.
  202 버스트는 세션이 언제 돌았는지 교차 확인하는 용도까지다. 그마저도 Preview 탭을 붙이기만 해도 `tools/list`가 돌며 202가 찍히고
