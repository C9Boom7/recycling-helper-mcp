...
매핑이 멀쩡해도 렌더링이 그 매핑을 안 거치면 소용없으므로, 카드 쪽은 smoke의 `runItemCardLabelSweep`이 따로 본다. 카드 머리말의 `- 키: 값` 줄을 품목 330개 전부에 대해 훑어 한글이 한 글자도 없는 값이 있으면 떨어뜨린다. 머리말에 새 줄을 붙일 때는 이 검사를 먼저 통과시켜야 한다.
