...
`bottle_sterilizer`의 `needsRegionCheck`는 `true`로 뒀다. 무상방문수거를 안내하면서 이 값을 `false`로 두면
지역 신청 경로가 통째로 사라진다 — PR #16 후속에서 8품목이 그 사고를 냈고,
`session-coordination.md`에 기록이 남아 있다.
(여섯 품목 전부가 그런 건 아니다. `seasoning_container`와 `toilet_brush`는 결론이 전국 공통이라
`national_default` + `needsRegionCheck: false`다.)
...
`안마 벨트`·`벨트 마사지기`·`복대 벨트`·`등산 벨트`·`자동차 벨트`가 전부 `belt` 79점으로 확정돼 의류수거함 안내를 받는다.
한 낱말로 붙여 쓴 `안전벨트`는 독립 낱말 조건에 안 걸려 not_found인데, 띄어 쓴 `자동차 벨트`는 걸린다 — 띄어쓰기가 답을 가른다.
