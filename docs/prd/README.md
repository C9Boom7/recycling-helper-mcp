...
Agentic Player 10 본선 추가 개발(2026-08-13 ~ 08-31)을 Phase 0~12(13개)로 나눈 작업 명세다.
...
**2026-08-23에 주최 측이 일정을 일주일 미뤘다.** 아래가 현행이다.

| 날짜 | 내용 |
| --- | --- |
| 8/31(월) | 본선용 MCP 서버 개발 완료 — 이후 오류 위주 수정만 가능 |
| 9/1(화)~9/11(금) | 카카오 QA. 오류 전달 시 빠른 수정 필요 |
| 9/14(월) | 코드 프리징 — 이후 서버 수정 불가 |
| 9/16(수)~10/12(월) | 본선 (툴 챌린지). 9/16 14:00 시작 예정. 서버 수정 불가, 다만 서비스를 못 쓸 정도의 오류는 카카오와 협의해 고칠 수 있다 |

옛 일정은 8/23 개발 완료 · 8/24~26 QA · 8/27 프리징 · 8/31~9/28 본선이었다. 그 전제로 쓴 서술이
아래 PRD 곳곳에 남아 있으니, 문서에서 날짜를 만나면 이 표를 기준으로 읽는다.

**기능 변경 마감(내부 게이트)도 8/21 → 8/29로 같이 옮긴다.** 개발 완료 이틀 전이라는 간격은 그대로 뒀다.
주최 측 표에는 없는 우리 게이트라, 마감이 밀린 만큼 기계적으로 따라 옮긴 값이다.
...
병렬화되지 않고 사람 손이 필요하다. 8/31까지 이 사이클을 여러 번 돌려야 하므로, Phase 1~3 완료를 기다리지 말고
...
- 툴 개수 3~10 권장 (현재 6개. `find_disposal_spots`는 인증키가 있을 때만 노출된다).
...
| 6 | 완료 — R0~R5 전부 | claude/phase6-r2-r4, claude/phase6-r5-early | 조례 10곳 1,176행 적재 — 수수료 보유 지역 8 → 18곳(비광역 전부), 행 849 → 2,025. R2 골든셋 69행 금액 불일치 0건, 파서 결함 둘을 고쳐 행 파싱 10/10. validate 규칙 3종·회귀 3건 추가. 코드 리뷰 후속으로 품목 오귀속 규칙을 조여 42행을 옮기거나 뺐다. **R5 1차(8/15 앞당겨 실행)**: 조례 개정 0곳, 링크 점검 범위를 수수료 파일까지 넓혀 117개로 확대하고 송파구 404·성남시 302를 고쳐 117/117. **R5 2차(8/31 출고 직전 재대조) 완료**: 조례 수집 14/14 성공, 시행일자 전부 일치(개정 0곳). 지역 링크 전수 점검에서 실제로 죽은 링크는 수원 1건뿐이라 이 PR에서 교체했다 |
...
| 11 | 완료 (PR #74 머지됨, 8/24) | claude/phase-10-followup-region-tool | [phase-11](phase-11-region-tool-response.md) — 지역 툴 응답 다이어트. `get_region_disposal_info` 노원구 매트리스 7,366B → 4,961B(−33%). 연락처 블록·수수료 행·품목별 안내 steps가 한 응답에 두 번 실리던 것을 걷고, `-` 규격이 카드와 cleanup plan에 새던 것과 두 툴 체크리스트가 갈리던 것을 닫았다 |
| 12 | 완료 (PR #77 머지됨, 8/24) | claude/phase-12-nearby-spots | [phase-12](phase-12-nearby-disposal-spots.md) — find_disposal_spots 신설. 키 없으면 미등록이 곧 스위치, 실패는 묶음표 자립 폴백. 여섯 번째 툴로 배포 서버에 노출 중인 것을 8/31 실측으로 확인했다 |
