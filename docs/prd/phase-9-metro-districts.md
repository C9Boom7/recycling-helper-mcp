...
- **서대문구 수집이 실패했다.** `https://www.sdm.go.kr/civil/print/waste/standards.do`가 302로 옮겨가 표가 없다. 이 PR이 건드리지 않은 파서(`sdm_table`)와 주소라 외부 변화다. 임포터가 그 지역을 건너뛰어 기존 101행은 그대로 남지만, **지금은 재현이 안 되는 데이터다.** Phase 6 R5 2차에서 같이 볼 항목이다.
