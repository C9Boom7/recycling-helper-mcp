...
`regionMatch`·`regionNotes`·수수료 요약 1줄은 `matchedItemWidget`이 한자리에서 모아 빌더 인자로 넘긴다 (빌더에서 재계산 금지). 카드를 쓰는 툴이 둘이라 핸들러마다 따로 계산하지 않는다.
...
- **확신도 안내도 함께 나간다.** 카드에만 붙이면 공유받은 사람에게는 medium 품목이 확정된 판단으로 도착한다(R2 7번과 같은 이유). 6줄 예산은 그대로 두고 그 한 줄만큼 steps 몫을 줄인다 — 제목 1줄 + 확신도 1줄이면 steps는 4줄, 넘치면 3단계 + 남은 단계 안내다. 카드 문구는 한 줄로는 길어서 공유본은 짧게 다시 쓴다.
