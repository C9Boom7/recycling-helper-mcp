...
- Source Gap 트랙: 현재 데이터 기준 `waste-items.json` 272개, `evaluation-cases.json` 272개, MCP answer cases 303개, review count는 `verified 39 / region_review_needed 84 / needs_source 7`이다. Phase 2 표준 티어 `standard_import` 142개가 여기에 더해진다. 남은 `needs_source` 7개는 공식 단독 근거가 없어 `source-gap-policy.md` 기준의 보수 안내로 유지한다.
