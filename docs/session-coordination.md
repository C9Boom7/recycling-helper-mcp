...
- Source Gap 트랙: 현재 데이터 기준 `waste-items.json` 324개, `evaluation-cases.json` 324개, MCP answer cases 443개, review count는 `verified 39 / region_review_needed 84 / needs_source 7`이다. 표준 티어 `standard_import` 194개(Phase 2의 142개 + Phase 7의 52개)가 여기에 더해진다. 남은 `needs_source` 7개는 공식 단독 근거가 없어 `source-gap-policy.md` 기준의 보수 안내로 유지한다.
