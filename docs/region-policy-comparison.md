...
### Standard tier — 경기 6개 시 (2026-08-16 추가)

`suwon_si`, `yongin_si`, `goyang_si`, `hwaseong_si`, `bucheon_si`, `namyangju_si`. 인구 순으로 잡았고, 여섯 곳 모두 완결 조건 5종을 채웠다. 성남시(`full`)까지 더해 경기 등록 지역은 7곳이다.

접수 창구가 시마다 갈린다. 수원·부천·남양주는 시가 직접 운영하는 신고 사이트(`waste.suwon.go.kr`, `clean.bucheon.go.kr`, `smartclean.nyj.go.kr`)를 갖고 있고, 고양은 고양도시관리공사의 `clean.gys.or.kr`을 쓴다 — 성남시가 성남도시개발공사의 `waste.isdc.co.kr`을 쓰는 것과 같은 형태다. 용인은 2025년 3월 자체 신청 폼을 닫고 '빼기'로 일원화해, 서울 중구·동대문구처럼 시청 배출신청안내 페이지를 `applicationUrl`로 쓴다. 화성은 청소행정포털 한 페이지에 신청 경로와 품목 수수료 검색이 함께 있다.

수수료표 형태도 다르다. 용인·부천·고양·남양주는 품목별 수수료가 **HTML 표**로 열려 있어 조례 파싱 없이 임포트할 수 있다. 수원도 HTML 표지만 인코딩이 euc-kr이다. 화성만 검색형이라 전체 표는 PDF로 받아야 한다.

LED 조명 기준이 시마다 다르니 주의한다. 화성은 평판형·십자형·원반형 LED를 대형폐기물로 보내고, 고양은 분리배출 대상이 아닌 LED를 종량제봉투로 넘긴다. 반면 **수원은 폐LED조명 재활용 시범사업 중이라 LED를 폐형광등 전용수거함으로 받고**, 남양주는 전구형·직관형과 컨버터 일체형·분리형 LED만 재활용 대상으로 가른다. "LED는 제외"라는 통념으로 뭉뚱그리면 안 되는 대목이다.

확인 못 한 두 곳은 데이터에 사실대로 적었다. 수원은 폐의약품 안내가 두 갈래로 갈린다 — 청소자원과는 수거함 운영을 말하고 보건소는 종량제봉투 배출을 안내한다. 부천은 폐형광등 배출 기준을 시 공식 페이지에서 찾지 못해 담당 부서 번호로 넘긴다.

링크 점검 때 눈여겨볼 것: 남양주 옛 도메인 `cleancity.nyj.go.kr`은 **접속이 끊겨 있는데도** 시청 재활용정보 페이지와 검색 결과가 아직 이 주소를 안내한다. 현행은 `smartclean.nyj.go.kr`이다.
