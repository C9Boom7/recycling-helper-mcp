...
`get_disposal_steps`의 품목 카드 텍스트는 **결론을 제목 바로 다음 줄에** 세웁니다. 카카오톡 말풍선에서 처음 두 줄이 가장 많이 읽히는 자리인데, 예전에는 거기에 `배출 그룹`·`확신도` 같은 분류 메타데이터가 서고 정작 답이 네 번째 줄로 밀려 있었습니다. `확신도`는 **높지 않을 때만** 냅니다(336개 중 255개가 `높음`이라 대부분의 답에서 아무것도 가르지 않고, 오히려 확신이 없다는 인상만 줍니다 — 위젯 카드가 이미 같은 규칙을 쓰고 있어 두 표면이 맞춰졌습니다). `판단 범위`는 아래 지역 블록(`지역 확인 필요`·`지역 참고`)이 서는 응답에서는 뺍니다 — 그 블록이 같은 말을 더 자세히 합니다.

이 편집은 품목 카드 텍스트에만 걸립니다. `classify_waste_item`의 텍스트 분기는 분류 요약이 그 툴의 일이라 `결론`·`확신도`·`판단 범위`를 그대로 냅니다.
