...
사진에서 알아본 이름은 사용자가 직접 친 이름보다 틀릴 확률이 높은데, 서버에 도착하는 건 어느 쪽이든 그냥 문자열입니다. 그래서 선택 파라미터 `inputSource: "photo"`로 그 차이를 받습니다. 이 값이 오면 확정 매칭 응답에 `사진 속 물건을 "○○"로 봤습니다. 다르면 품목명을 알려주세요.` 한 줄이 붙습니다(카드는 제목 바로 아래, 텍스트는 맨 앞). 잘못 알아본 답을 확정된 답으로 받게 두는 것보다 낫다는 판단입니다. 공유용 `copy_text`에는 넣지 않습니다 — 공유받는 사람은 사진을 보낸 적이 없어서 맥락이 없습니다. 값은 `photo` 하나뿐이고, 호출 로그에도 `inputSource`로 남겨 사진 입력이 실제로 얼마나 들어오는지 셉니다.
